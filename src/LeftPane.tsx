...
export function LeftPane({ selected, onSelect }: {
    ...
    return <div class="LeftPane">
        ...
        <TextInputAndButton iconName="add" value={invite} onChange={setInvite} buttonContent="Invite" placeholder="Tag"
            onClick={() => {
                if (invite) {
                    setInvite("");
        ...
        <div className="conversations">
            {chatService.inbox.conversations.map(x =>
                <ConversationCard key={x.channelId} conversation={x} selected={x === selected} onSelect={() => onSelect(x)} />)}
        </div>
    </div>
}